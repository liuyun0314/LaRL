...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, capacity):
        self.episode_length = capacity
        ...
        act_shape = args.num_new_jobs + args.num_tasks * args.num_warmup_jobs + 1
    ...
    def sample(self, batch_size, buffer):
        """
        Sample a batch of data from the buffer.
        :return: (dict) batch of data.
        """
        if self.counter >= self.episode_length:
            max_step = self.episode_length
        else:
            max_step = self.counter
        indices = np.random.choice(max_step, batch_size, replace=False)
        buffer.share_obs[:] = self.share_obs[indices]
        buffer.obs[:] = self.obs[indices]
        buffer.value_preds[:] = self.value_preds[indices]
        buffer.returns[:] = self.returns[indices]
        buffer.advantages[:] = self.advantages[indices]
        buffer.available_actions[:] = self.available_actions[indices]
        buffer.actions[:] = self.actions[indices]
        buffer.action_log_probs[:] = self.action_log_probs[indices]
        buffer.rewards[:] = self.rewards[indices]
        buffer.masks[:] = self.masks[indices]
        buffer.bad_masks[:] = self.bad_masks[indices]
        buffer.active_masks[:] = self.active_masks[indices]
        return buffer
