...
class MultiTD3(object):
    ...
    def __init__(self, id, algo_name, state_dim, action_dim, hidden_size, actor_lr, critic_lr, gamma, tau, savetag,
                 foldername, use_gpu, num_agents, init_w=True):
    ...
    def update_parameters(self, state_batch, next_state_batch, action_batch, reward_batch, done_batch, global_reward,
                          agent_id, num_epoch=1, **kwargs):
        """
        ...
        for _ in range(num_epoch):
            ...
            with torch.no_grad():
                ...
                # Policy Noise
                policy_noise = np.random.normal(0, kwargs['policy_noise'],
                                                (action_batch.size()[0], action_batch.size()[1]))
                policy_noise = torch.clamp(torch.Tensor(policy_noise), -kwargs['policy_noise_clip'],
            ...
            if self.num_critic_updates % kwargs['policy_ups_freq'] == 0: utils.soft_update(self.policy_target,
                                                                                           self.policy, self.tau)
...
class MATD3(object):
    ...
    def __init__(self, id, algo_name, state_dim, action_dim, hidden_size, actor_lr, critic_lr, gamma, tau, savetag,
        ...
        self.agent_id = id
        self.use_gpu = use_gpu
        self.tracker = utils.Tracker(foldername, ['q_' + savetag, 'qloss_' + savetag, 'policy_loss_' + savetag], '.csv',
                                     save_iteration=1000, conv_size=1000)
        ...
        self.critics = [QNetwork(state_dim * num_agents, action_dim * num_agents, hidden_size * 3) for _ in
                        range(num_agents)]

        self.critics_target = [QNetwork(state_dim * num_agents, action_dim * num_agents, hidden_size * 3) for _ in
                               range(num_agents)]
        if init_w:
        ...
        for _ in range(num_epoch):
            ...
            with torch.no_grad():
                ...
                if self.algo_name == 'TD3':
                    # Policy Noise
                    policy_noise = np.random.normal(0, kwargs['policy_noise'], (
                    action_batch.size()[0], action_batch.size()[1] * action_batch.size()[2]))
                    policy_noise = torch.clamp(torch.Tensor(policy_noise), -kwargs['policy_noise_clip'],
                                               kwargs['policy_noise_clip'])
            ...
            current_q1, current_q2 = self.critics[agent_id].forward((state_batch.view(batch_size, -1)),
            ...
            if self.num_critic_updates % kwargs['policy_ups_freq'] == 0 or self.algo_name == 'DDPG':
                ...
                Q1, Q2 = self.critics[agent_id].forward(state_batch.view(batch_size, -1),
            ...
            if self.num_critic_updates % kwargs['policy_ups_freq'] == 0 or self.algo_name == 'DDPG': utils.soft_update(
                self.policy_target, self.policy, self.tau)
            for critic, critic_target in zip(self.critics, self.critics_target):
