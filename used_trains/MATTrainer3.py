...
from model.policy3 import TransformerPolicy as Policy
from torch.optim import Adam
from LLMforReward.reward_model import Reward_Model
...
class MATTrainer:
    ...
    def __init__(self, args, num_agents, id, device):

        self.device = device
        self.args = args
        self.tpdv = dict(dtype=torch.float32, device=device)
        self.num_agents = num_agents
        action_dim = args.num_new_jobs + args.num_warmup_jobs
        ...
        self.reward_model = LLMRewardDecomposer(args, id)
    ...
    def ppo_update(self, sample):
        ...
        action_log_probs, values, dist_entropy = self.policy.evaluate_actions(share_obs_batch,
                                                                              obs_batch,
                                                                              actions_batch,
    ...
    def ppo_update1(self, sample):
        ...
        # Reshape to do in a single forward pass for all steps
        action_log_probs, _, dist_entropy = self.policy.evaluate_actions(share_obs_batch,
                                                                              obs_batch,
                                                                              actions_batch,
                                                                              available_actions_batch,
                                                                              active_masks_batch)
        ...
        if self._use_policy_active_masks:
            policy_loss = (-torch.sum(torch.min(surr1, surr2), dim=-1,
        ...
        return grad_norm, policy_loss, dist_entropy, grad_norm, imp_weights

    def train1(self, buffer):
        """
        Perform a training update using minibatch GD.
        :param buffer: (SharedReplayBuffer) buffer containing training data.
        :param update_actor: (bool) whether to update actor network.
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights \
                    = self.ppo_update1(sample)
                train_info['policy_loss'] += policy_loss.item()
                train_info['dist_entropy'] += dist_entropy.item()
                train_info['ratio'] += imp_weights.mean()
