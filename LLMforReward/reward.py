...
class LLMRewardDecomposer(RRDRewardDecomposer):
    def __init__(self, args):
        ...
        self.rd_save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.args.llm_response_dir,
                                        self.args.env, str(self.args.seed))
        self.id = 0
    ...
    def load_rd_functions(self):
        ...
        while not os.path.exists(os.path.join(self.rd_save_dir, f'response_{self.id}.npy')):
            callgpt(self.args.env, self.rd_save_dir, True, self.id, n=self.args.llm_n, port=self.args.port,
                    seed=self.args.seed, factor=(not self.args.direct_generate))
